...
function obj:_build_repo_json_url(repo)
	if self.repos[repo] and self.repos[repo].url then
		...
		self.repos[repo].json_url = string.gsub(self.repos[repo].url, "/$", "")
			...
			.. branch
			.. "/docs/docs.json"
		self.repos[repo].download_base_url = string.gsub(self.repos[repo].url, "/$", "")
			.. "/raw/"
			.. branch
			.. "/Spoons/"
		return true
...
function obj:search(pat)
	...
	for repo, v in pairs(self.repos) do
		if v.data then
			...
			self.logger.ef(
				"Repository data for '%s' not available - call spoon.SpoonInstall:updateRepo('%s'), then try again.",
				repo,
				repo
			)
		end
...
function obj:_installSpoonFromZipURLgetCallback(urlparts, callback, status, body, headers)
	...
	if (status < 100) or (status >= 400) then
		...
		local tmpdir =
			_x("/usr/bin/mktemp -d", "Error creating temporary directory to download new spoon.")
		if tmpdir then
...
function obj:asyncInstallSpoonFromZipURL(url, callback)
	...
	if dlfile and dlfile ~= "" and urlparts.pathExtension == "zip" then
		hs.http.asyncGet(
			url,
			nil,
			hs.fnutils.partial(self._installSpoonFromZipURLgetCallback, self, urlparts, callback)
		)
		return true
	else
