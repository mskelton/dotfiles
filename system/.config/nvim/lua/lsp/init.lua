...
--- Root directory callback that only activates LSP when a marker is found.
...
M.server = function(server, config)
	...
	--- `enabled` is our own flag, not an LSP client field.
	config.enabled = nil
	...
	M.server("oxfmt", {
		cmd = function(dispatchers, config)
			...
			if config and config.root_dir then
				local local_bin = vim.fs.joinpath(config.root_dir, "node_modules/.bin", "oxfmt")
